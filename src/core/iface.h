...
static inline int core_process(const u8 *req_buf, size_t req_buflen, u8 **res_buf, size_t *res_buflen)
{
...
LAB_OUT:
    ...
    *res_buf = kzmalloc(*res_buflen, GFP_KERNEL);
    ...
    ((struct raw_packet_res*)(*res_buf))->status.type = retv;
    ((struct raw_packet_res*)(*res_buf))->status.domain = domain;

    if (cmd_res_buf)
    {
        memcpy(((struct raw_packet_res*)(*res_buf))->content, cmd_res_buf, cmd_res_buflen);
        kzfree(cmd_res_buf, cmd_res_buflen);
    }

    NETKIT_LOG("[*] res_buf->status: %d\n", ((struct raw_packet_res*)(*res_buf))->status.type);

    return ((struct raw_packet_res*)(*res_buf))->status.type;
}
